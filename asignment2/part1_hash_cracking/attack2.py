...
def crack_single_entry(salt, target_hash, algo, wordlist):
...
def crack_salted_hashes_single_threaded(salted_entries, algo, wordlist):
    
    start_time = time.time()
    cracked_results = []

    for entry in salted_entries:
        salt, target_hash = entry
        result = crack_single_entry(salt, target_hash, algo, wordlist)
...
def process_algorithm(hash_file, algo, wordlist):
    ...
    cracked, elapsed_time = crack_salted_hashes_single_threaded(
        salted_entries, algo, wordlist
    )
